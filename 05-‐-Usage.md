...
Refer to the [**Certipy Command Reference**](08-%E2%80%90-Command-Reference) for a complete list of commands and options. The following sections will cover the most relevant commands for AD CS abuse, including their usage and examples.
...
2. **Request a certificate** - Suppose the find output showed **ESC1** on a template that Domain Users can enroll. The attacker can attempt to exploit it:
...
3. **Authenticate with the certificate** - Now the attacker has `administrator.pfx`. They use `certipy auth` to leverage it:
